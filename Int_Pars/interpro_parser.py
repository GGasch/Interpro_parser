...
def get_args():
    parser = argparse.ArgumentParser(
        description='Based on fasta and tsv file downloaded from Intepro, extract the portion of the sequence that matches the query domain')
...
def stat(df):
    ...
    df_bound:pd.DataFrame = df["Matches"].str.split(",")
    df_len = df_bound.apply(lambda x: [int(i.split('..')[1]) - int(i.split('..')[0]) for i in x])
    list_len:list = sum(df_len.tolist(), [])  # Transfer the dataframe in a list. The sum is here to denest the list
    ...
    with open(fasta_file) as fasta:
        for record in tqdm(SeqIO.parse(fasta, "fasta")):
            ...
            if name in df[
                ...
                bounds_0 = df.loc[df['Accession'] == name, 'Matches'].iloc[0].split(
                    ',')  # Extract the bounds (upper and lower) of all the domains present in the sequence
                bounds_1 = [j.split('..') for j in
                            bounds_0]  # Turn the couple into a list of number that are easier to handle
                ...
                for bounds in bounds_1:
                    if int(bounds[1]) - int(bounds[0]) >= avg - 2 * sd:
                        ...
                        record_crpd = SeqRecord(seq_crpd, record.id, "",
                                                "")  # Create the element of the future fasta file
                        list_record_crpd.append(record_crpd)
...
def main():
    ...
    df_in = pd.read_csv(args.tsv, sep='\t', index_col=False)  # Transfer the tsv file into a pandas file
    if args.trim:
        avg, sd = stat(df_in)
        print(avg, sd)
